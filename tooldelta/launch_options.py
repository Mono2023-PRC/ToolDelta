    ...
    try:
        ...
        while True:
            if launch_section:
                ...
                launch_choices.extend(
                    (key, f"{key} - {opt_name}")
                    for key, (opt_name, _) in more_opts.items()
                )
                launch_choices.append(("q", "q - 退出"))
                selected = tui_bridge.select("请选择: ", launch_choices)
                if selected is None:
                    fmts.clean_print(
                ...
                r = (selected if selected is not None else input("请选择: ")).strip()
            launch_section = ""
            match r:
                case "1":
                    start_tool_delta()
